				...
				unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_darkness_cpuinfo->prev_cpu_user = (kcpustat_cpu(cpu).cpustat[CPUTIME_USER]);
		this_darkness_cpuinfo->prev_cpu_system = (kcpustat_cpu(cpu).cpustat[CPUTIME_SYSTEM]);
		this_darkness_cpuinfo->prev_cpu_others = (kcpustat_cpu(cpu).cpustat[CPUTIME_IRQ] + kcpustat_cpu(cpu).cpustat[CPUTIME_SOFTIRQ]
																	+ kcpustat_cpu(cpu).cpustat[CPUTIME_STEAL] + kcpustat_cpu(cpu).cpustat[CPUTIME_NICE]);

		this_darkness_cpuinfo->prev_cpu_idle = (kcpustat_cpu(cpu).cpustat[CPUTIME_IDLE]);
		this_darkness_cpuinfo->prev_cpu_iowait = (kcpustat_cpu(cpu).cpustat[CPUTIME_IOWAIT]);
